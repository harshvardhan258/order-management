const orders = [
  {
    orderId: 1001,
    vendorName: "ABC Company",
    pickupDate: "2023-04-28",
    status: "Shipped",
  },
  {
    orderId: 1002,
    vendorName: "XYZ Enterprises",
    pickupDate: "2023-04-30",
    status: "Pending",
  },
  {
    orderId: 1003,
    vendorName: "LMN Corporation",
    pickupDate: "2023-05-01",
    status: "Cancelled",
  },
  {
    orderId: 1004,
    vendorName: "PQR Limited",
    pickupDate: "2023-05-02",
    status: "Pending",
  },
  {
    orderId: 1005,
    vendorName: "DEF Corporation",
    pickupDate: "2023-05-03",
    status: "Shipped",
  },
  {
    orderId: 1006,
    vendorName: "GHI Enterprises",
    pickupDate: "2023-05-05",
    status: "Pending",
  },
  {
    orderId: 1007,
    vendorName: "JKL Limited",
    pickupDate: "2023-05-06",
    status: "Cancelled",
  },
  {
    orderId: 1008,
    vendorName: "MNO Company",
    pickupDate: "2023-05-07",
    status: "Shipped",
  },
  {
    orderId: 1009,
    vendorName: "STU Enterprises",
    pickupDate: "2023-05-08",
    status: "Pending",
  },
  {
    orderId: 1010,
    vendorName: "VWX Corporation",
    pickupDate: "2023-05-09",
    status: "Shipped",
  },
  {
    orderId: 1011,
    vendorName: "YZ Company",
    pickupDate: "2023-05-10",
    status: "Pending",
  },
  {
    orderId: 1012,
    vendorName: "ABC Enterprises",
    pickupDate: "2023-05-11",
    status: "Cancelled",
  },
  {
    orderId: 1013,
    vendorName: "LMN Limited",
    pickupDate: "2023-05-12",
    status: "Pending",
  },
  {
    orderId: 1014,
    vendorName: "PQR Corporation",
    pickupDate: "2023-05-14",
    status: "Shipped",
  },
  {
    orderId: 1015,
    vendorName: "DEF Enterprises",
    pickupDate: "2023-05-15",
    status: "Pending",
  },
  {
    orderId: 1016,
    vendorName: "GHI Limited",
    pickupDate: "2023-05-17",
    status: "Shipped",
  },
  {
    orderId: 1017,
    vendorName: "JKL Company",
    pickupDate: "2023-05-18",
    status: "Cancelled",
  },
  {
    orderId: 1018,
    vendorName: "MNO Enterprises",
    pickupDate: "2023-05-19",
    status: "Shipped",
  },
  {
    orderId: 1019,
    vendorName: "STU Corporation",
    pickupDate: "2023-05-20",
    status: "Pending",
  },
  {
    orderId: 1020,
    vendorName: "VWX Company",
    pickupDate: "202",
    status: "Pending",
  },
];
